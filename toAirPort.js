...
function checkFareCalcStart(){
	if(document.getElementById("inpCity").value == ""){
		alert("Need to give the address");
	}
	else{
		window.calcFare();
	}
...
function verifyAndSendSMS(){
	if (document.getElementById("FareText").innerHTML == "Fare = nil") {
		alert("Estimate cannot be forwarded with given details");
	}
	else{
		var popup;
        popup = window.open("Popup-SMS-Verification.html", "Popup", "width=400,height=700");
        popup.focus();
	}
